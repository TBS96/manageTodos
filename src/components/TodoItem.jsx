...
const TodoItem = ({eachTodoObj}) => {
    ...
    return (
        <div className={`flex border-black/10 rounded-lg px-3 py-1.5 gap-x-3 shadow-sm shadow-white/50 duration-300 text-black ${eachTodoObj.completed ? "bg-[#c6e9a7]" : "bg-[#b57edf]"}`}>
            ...
            {/* editable text */}
            <input type="search" className={`border cursor-pointer outline-none w-full bg-transparent rounded-lg ${isTodoEditable ? "border-black/10 px-2 bg-white/50" : "border-transparent"} ${eachTodoObj.completed ? "line-through" : ""}`} value={todoMsg} onChange={(e) => setTodoMsg(e.target.value)} readOnly={!isTodoEditable} onClick={toggleDone} />
